...
if(len(sys.argv)>1):
    ...
    if (len(sys.argv)>3 and sys.argv[3]=="backtest"):
        backtest = True
    if (len(sys.argv)>4):
        filename =sys.argv[4]
...
cerebro.addstrategy(StrategyStore.Strategy,minPrice=minPrice,maxPrice=maxPrice,
    btc_sup=btc_sup,btc_res=btc_res,buyamount=buyamount,tp=tp,sl=sl,marketplace=marketplace,backtest=backtest)
...
if backtest :
    ...
    cerebro.adddata(CCXTFeed(dataname=currency+'/'+marketplace, name=currency+"_1min",
    ...
    cerebro.adddata(CCXTFeed(dataname='BTC/'+marketplace, name="BTC_1min",
                        timeframe=bt.TimeFrame.Minutes, fromdate=fromdate,todate=todate,
                        compression=1,exchange='binance', ohlcv_limit=1440,currency=currency, config=config, retries=5 ))
    cerebro.adddata(CCXTFeed(dataname=currency+'/'+marketplace, name=currency+"_4h",
                        timeframe=bt.TimeFrame.Minutes, fromdate=fromdate,todate=todate,
